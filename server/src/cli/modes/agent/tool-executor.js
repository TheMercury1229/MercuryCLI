...
export class ToolExecutor {
  ...
  /** @param {string} rel @param {string} op */
  ...
  /** @param {string} rel @param {boolean} recursive */
  ...
  /** @param {string} rootRel @param {string} globPattern @param {string} [contentQuery] */
  searchFiles(rootRel, globPattern, contentQuery) {
    ...
    else results.push(path.relative(this.config.codebasePath, rootAbs).split(path.sep).join("/"));
  ...
  analyzeCodebase(rootRel) {
    ...
    const walk = (dir) => {
      for (const ent of fs.readdirSync(dir, { withFileTypes: true })) {
        ...
        const relP = path.relative(this.config.codebasePath, full);
        if (this.#excluded(relP)) continue;
        if (ent.isDirectory()) { dirs++; walk(full); }
        else files++;
      }
    };
    if (fs.statSync(rootAbs).isDirectory()) walk(rootAbs);
  ...
  applyApprovedFromTracker() {
    ...
    for (const a of all.filter((x) => x.type === "folder_create" && x.status === "approved")) {
      try { fs.mkdirSync(this.#resolveSafe(a.path), { recursive: true }); }
